...
<script type="text/javascript">
	...
	function addComment() {
		...
		var num = $('#num').val();
		$.ajax({
			url : "bbs1InsertReplyController",
			method : "POST",
			data : {
				num : num,
				bbs1ReplyContent : comment
			},
	...
	function selectBbs1ReplyFunction() {
		$('#sortable').val("");
		var num = $('#num').val();
		$.ajax({
			url : "bbs1ShowReplyController",
			method : "POST",
			data : {
				num : num
			},
...
<%
	...
	if (session.getAttribute("id") != null) {
		...
		title = (String) request.getAttribute("bbs1Title");
		content = (String) request.getAttribute("bbs1Content");
		writer = (String) request.getAttribute("bbs1Nickname");
		date = (String) request.getAttribute("bbs1Date");
		bbsId = (String) request.getAttribute("bbs1Id");
				...
				<div id="view" style="padding-left: 15%">
					...
					<div class="mx-auto"
						...
						<button type="button" class="btn btn-secondary" id="btn"
							onclick="location.href='bbs1Mod.jsp?num=<%=no%>&title=<%=title%>&content=<%=content%>'">수정</button>
						&nbsp;&nbsp;
						<button type="button" class="btn btn-secondary" id="btn"
							onclick="location.href='bbs1DeleteController?num=<%=no%>'">삭제</button>
						<%
