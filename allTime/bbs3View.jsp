...
<script type="text/javascript">
	...
	function addComment() {
		...
		var num = $('#num').val();
		$.ajax({
			url : "bbs3InsertReplyController",
			method : "POST",
			data : {
				num : num,
				bbs3ReplyContent : comment
			},
	...
	function selectBbs3ReplyFunction() {
		$('#replyList').val("");
		var num = $('#num').val();
		$.ajax({
			url : "bbs3ShowReplyController",
			method : "POST",
			data : {
				num : num
			},
		...
		$('#replyList')
				.append(
						'<h5 style="color:black;">'
								...
								+ comment
								+ '<br></strong><small class="pull-right text-muted"> <span'
							+ '   class="glyphicon glyphicon-time"></span>'
								+ date
								+ '</small><li class="ui-state-default">'
								+ '<input type="button" class="btn btn-secondary"  id="XBtn"'
								+ 'value ="삭제" style="font-size:x-small" onclick = "location.href=\'bbs3DeleteReplyController?name='
...
<%
	...
	if (session.getAttribute("id") != null) {
		...
		title = (String) request.getAttribute("bbs3Title");
		content = (String) request.getAttribute("bbs3Content");
		writer = (String) request.getAttribute("bbs3Nickname");
		date = (String) request.getAttribute("bbs3Date");
		bbsId = (String) request.getAttribute("bbs3Id");
		college = (String) session.getAttribute("college");
		classtitle = (String) request.getAttribute("bbs3ClassTitle");
		classteacher = (String) request.getAttribute("bbs3ClassTeacher");
	}
%>
<body>
	<!-- nav bar -->
	...
	<div id="body">
		...
		<div class="container-fluid">
			...
			<!-- Breadcrumbs-->
			<ol class="breadcrumb">
				<li class="breadcrumb-item active">강의평가 게시판</li>
			</ol>
		...
		<div class="how-section1"
			...
			<h1 style="font-family: 'Hanna'; margin-left: 20%; color: #112f41;"><%=classtitle%>
				/
				<%=classteacher%></h1>
				<div class="col-md-6">
					<h2
				...
				<div class="mx-auto"
					...
					<button type="button" class="btn btn-secondary" id="btn"
						onclick="location.href='bbs3Mod.jsp?num=<%=no%>&title=<%=title%>&content=<%=content%>'">수정</button>
					&nbsp;&nbsp;
					<button type="button" class="btn btn-secondary" id="btn"
						onclick="location.href='bbs3DeleteController?num=<%=no%>'">삭제</button>
					<%
					...
					<div class="well">
						...
						<div class="input-group">
							<input type="hidden" id="num" value=<%=no%> /> <input
								type="text" id="reply" class="form-control input-sm chat-input"
